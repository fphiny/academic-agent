...
class RetrievalFlow:
    ...
    def run(
        ...
        *,
        ...
        extracted_documents = yield from self._run_external_document_phase(
            original_user_message=original_user_message,
        ...
        *,
        ...
        for atomic_idx, atomic_query in enumerate(realized_atomic_queries, start=1):
            ...
            for ridx, rewritten_query in enumerate(rewritten_queries, start=1):
                ...
                if self.external.has_meaningful_search_items(search_result.data):
            ...
            collected_items = self.sources.collect_search_items_from_result(chosen_result_data)
            if collected_items:
            ...
            yield {
                ...
                "step": 2,
                "delta": (
                    f"[외부 검색 채택 {atomic_idx}/{len(realized_atomic_queries)}] "
                    f"질문={json.dumps(atomic_query, ensure_ascii=False)} | "
                    f"채택 검색어={json.dumps(adopted_query, ensure_ascii=False)} | "
