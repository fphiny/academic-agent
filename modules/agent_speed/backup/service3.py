...
class AgentSpeedService:
    ...
    def _decompose_user_message_into_atomic_queries(
        ...
        *,
        ...
        system_prompt = (
            "너는 사용자 질문을 가능한 한 원문 그대로 유지하는 질의 정리기다.\n"
            "기본적으로 질의를 분해하지 않는다.\n"
            "사용자가 직접 입력하지 않은 대상명, 캠퍼스명, 사람명, 장소명은 추가하지 마라.\n"
            "원문이 단일 질의로 충분하면 그대로 1개만 반환한다.\n"
            "서로 독립된 대상이 사용자의 원문에 명시적으로 둘 이상 있을 때만 분해한다.\n"
            "\n"
            "[출력 규칙]\n"
            "- 반드시 JSON만 출력\n"
    ...
    def _build_search_queries_with_llm(
        ...
        *,
        ...
        system_prompt = (
            "너는 검색어 생성기다.\n"
            "사용자의 입력을 웹 검색용 질의로 변환한다.\n"
            "반드시 JSON만 출력한다.\n"
            "\n"
            "[목표]\n"
            "- 입력 문장을 검색엔진용 짧은 질의로 축약\n"
            "- 최대 10개\n"
            "- 입력 의미를 보존\n"
            "\n"
            "[절대 규칙]\n"
            "1) 입력에 없는 단어는 추가하지 말 것.\n"
            "2) 입력의 일반 명사를 특정 하위 항목으로 구체화하지 말 것.\n"
            "3) 입력에 없는 복수 대상이나 목록을 추론해서 만들지 말 것.\n"
            "4) 검색 최적화를 이유로 의미 확장, 예시 추가, 자동 보완 금지.\n"
            "5) 불확실하면 추가하지 말고, 입력 단어를 줄이는 방향으로만 처리.\n"
            "\n"
            "[생성 규칙]\n"
            "1) 군더더기 제거\n"
            "2) 조사/어미 제거\n"
